<?php
    ...
    //showing php errors for the following code
    ...
    // protecting the password for hashing the password
    $hashedPassword = password_hash($password, PASSWORD_DEFAULT);

    // Inserting data into users table query with placeholders
    $query = "INSERT INTO Users (firstname, lastname, DOB, username, password, email, Phone_Number, is_admin) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = mysqli_prepare($con, $query);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "sssssss", $firstname, $lastname, $dob, $username, $hashedPassword, $email, $number, $userRole);

        if (mysqli_stmt_execute($stmt)) {

            // Send registration email using PHPMailer if the query is successuful
            ...
            try {
                ...
                // the body of the mail which will be send to the user.
                $mail->isHTML(true);
                $mail->Subject = 'Registration Successful';
                $mail->Body    = "Dear $username,<br><br>Thank you for registering at 92 Donner Kebab. Your registration is successful.";

                $mail->send();

                // Notifing users if the registration was success or error.
...
<html lang="en">
    ...
    <script>
        //this code is for the zoom function
    let scale = 1;
        function zoomIn() {
        scale += 0.1;
        updateZoom();
        ...
        <!-- this code is to link with bootstrap js and css -->
        ...
        // the following function to show content based on language
     ...
     <script>
    //the following code if for the text to speech fucntion.
    let speechUtterance;
    ...
    <body>
        ...
        <div id="content">
        <!-- this id is for the language function-->
        <div id="eng-content" style="display: none;">
        <center>
        <!-- the following is the header and navbar of the page-->
    ...
    <div class="container my-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb breadcrumb-chevron p-3 bg-white rounded-3">
                ...
                <li class="breadcrumb-item">
                    <a class="link-body-emphasis fw-semibold text-decoration-none" href="staff.php">New Staff</a>
                </li>
                <li>
                    <!-- the follwong code is for the language buttons, zoom buttons and text to speech buttons-->
...
<!-- the following code is for the french language content-->
<div id="fr-content">
<div id="content">
<center>
